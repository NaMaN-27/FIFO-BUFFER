#include "isim/work/fifo__test__v/fifo__test__v.h"
#include "isim/work/glbl/glbl.h"
static const char * HSimCopyRightNotice = "Copyright 2004-2005, Xilinx Inc. All rights reserved.";


#include "work/fifo__test__v/fifo__test__v.h"
static HSim__s6* IF0(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new workMfifo__test__v(label); 
    return blk;
}


#include "work/fifo/fifo.h"
static HSim__s6* IF1(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new workMfifo(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("fifo_test_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("fifo", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        topvl = new workMfifo__test__v("fifo_test_v");
